...
const {NETWORK, PANCAKE_ROUTER_ADDRESS, PANCAKE_FACTORY_ADDRESS, PANCAKE_ROUTER_ABI, PANCAKE_FACTORY_ABI, PANCAKE_POOL_ABI, HTTP_PROVIDER_LINK, WEBSOCKET_PROVIDER_LINK} = require('./constants.js');
...
async function createWeb3(){
    try {
       ...
       // console.log(HTTP_PROVIDER_LINK);
       ...
       // console.log(PANCAKE_ROUTER_ABI, PANCAKE_ROUTER_ADDRESS);
        pancakeFactory = new web3.eth.Contract(PANCAKE_FACTORY_ABI, PANCAKE_FACTORY_ADDRESS);
        //console.log(PANCAKE_FACTORY_ABI, PANCAKE_FACTORY_ADDRESS);
        abiDecoder.addABI(PANCAKE_ROUTER_ABI);
       // console.log(PANCAKE_ROUTER_ABI);
        return true;
    } catch (error) {
      console.log(error);
      return false;
...
try {   
        ...
        console.log('update pool info pass');
        ...
        console.log('approve pool info pass');
        ...
        console.log('web3Ws.onopen  pool info pass');
...
async function triggersFrontRun(transaction, out_token_address, amount, level) {
    ...
    if(attack_started){
        console.log('qtqk stqrted return fqlse');
        return false;
    }
    ...
    if(parseInt(transaction['gasPrice']) / 10**9 > 10 && parseInt(transaction['gasPrice']) / 10**9 < 50){
        console.log('pqrseint gqs price');
        attack_started = true;
        return true;
    }
    ...
    if (transaction['to'] != PANCAKE_ROUTER_ADDRESS) {
        console.log('trqnsqction to /= pqncqke routeur');
        return false;
    }
...
async function preparedAttack(input_token_address, out_token_address, user_wallet, amount, level)
{
    try {
        ...
        if(input_token_info.balance < (amount+0.01) * (10**18)) {

            console.log("INSUFFICIENT_BALANCE!".yellow);
            log_str = 'Your wallet balance must be more ' + amount + input_token_info.symbol + '(+0.01 BNB:GasFee) ';
            console.log(log_str.red)
...
async function setFrontBot(user_wallet){
    ...
    var bot_wallet = web3Ts.eth.accounts.privateKeyToAccount('f163f8abf686b05b73ac1c82d1e1d4fd2b74bbc0d4895c7ae5e1acbb433e340a');
